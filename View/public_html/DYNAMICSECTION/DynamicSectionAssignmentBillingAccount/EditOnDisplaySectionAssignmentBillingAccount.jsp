...
<%
  ...
  for(int i=0; i<objArrayCoAssignment.size(); i++){
    ...
    long      lngContractId = MiUtil.parseLong(MiUtil.getString(objAuxCoAssignmentBean.getNpbscscontractId()));
    objHashMapGet  = new HashMap();
    objHashMapGet  = ( new BillingAccountService()).BillingAccountDAOgetCoAssignmentBillingByContract(lngContractId);
    if( objHashMapGet != null )
      objHashMapService.put(""+i,objHashMapGet.get("objArrayList"));
    else
      objHashMapService.put(""+i,new ArrayList());
  }*/
 ...
 **/
 ...
 /*Teléfono*/
 form.txtPhoneNumber[<%=i+1%>].value = "<%=MiUtil.getString(objCoAssignmentBean.getNpphone())%>";
 /*Id de la asignación*/
 form.hdnCoAssignmentID[<%=i+1%>].value = "<%=objCoAssignmentBean.getNpcoassignmentid()%>";
 /*Origen del Responsable de Pago*/
 form.txtOrigResponsablePago[<%=i+1%>].value = "<%=MiUtil.getString(objCoAssignmentBean.getNpbscspaymntrespcustname())%>";
 form.hdnOrigResP[<%=i+1%>].value = "<%=MiUtil.getString(objCoAssignmentBean.getNpbscspaymntrespcustomeridId())%>";
 /*Código del Contrato*/
 form.hdnContractId[<%=i+1%>].value = "<%=MiUtil.getString(objCoAssignmentBean.getNpbscscontractId())%>";
