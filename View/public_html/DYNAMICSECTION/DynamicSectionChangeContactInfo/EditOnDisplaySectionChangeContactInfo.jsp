...
<% 
 try{
  Hashtable hshtinputEditSection = null;
  ...
  hshtinputEditSection     = (Hashtable)request.getAttribute("hshtInputEditSection");
  
  System.out.println("hshtinputEditSection ["+hshtinputEditSection);
  
  if ( hshtinputEditSection != null ) {
  
    strCustomerId           =   (String)hshtinputEditSection.get("strCustomerId");
    strnpSite               =   (String)hshtinputEditSection.get("strSiteId");
    strCodBSCS              =   (String)hshtinputEditSection.get("strCodBSCS");
    hdnSpecification        =   (String)hshtinputEditSection.get("strSpecificationId");
    //strSolution             =   (String)hshtinputNewSection.get("strSolution");
    strTypeCompany          =   (String)hshtinputEditSection.get("strTypeCompany");

    strOrderId              =   (String)hshtinputEditSection.get("strOrderId");

	  strGeneratorId      		= 	(String)hshtinputEditSection.get("strGeneratorId");
	  strGeneratorType  	  	=	  (String)hshtinputEditSection.get("strGeneratorType");	
		
	  System.out.println("[EditOnDisplaySectionChangeContactInfo.jsp]strGeneratorId: "+strGeneratorId);
	  System.out.println("[EditOnDisplaySectionChangeContactInfo.jsp]strGeneratorType: "+strGeneratorType);
...
%>
       ...
       var index = form.cmbTitulo.length;
         ...
         for(i=0; i <vctTitulo.size(); i++){
          ...
          if(index == 1 && form.cmbTitulo.options!= undefined ) {
            ...
            //form.cmbTitulo.option = new Option( objTitulo.titleName , objTitulo.titleId );
           } else {
  ...
  <script DEFER>
     function fxGetNombres(){
        return "<td><input type=text name=txtNombres size=15 onchange=this.value=trim(this.value.toUpperCase()) /> <input type=hidden name=hdnNombresOld /> <input type=hidden name=hdnTituloOld /> <input type=hidden name=hdnCargoNameOld /> <input type=hidden name=hdnPersonId /> <input type=hidden name=hdmCargoDescrip /> <input type=hidden name=hdnNpitemid /> <input type=hidden name=hdnNpaction value = 'Nuevo' /> </td>";    	
     }
  </script>  
  <script DEFER>
  ...
  <script DEFER>
   ...
   <script DEFER>
     function fxGetEstatus(){
       var cadena = "<td>"
       cadena+="<select value=0 name=cmbEstatus disabled onchange='javascript:fxOnChangeStatus(this,this.parentNode.parentNode.rowIndex);'  >";
       cadena+="<option>";
       cadena+="&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
       cadena+="</option>";
       cadena+="</select>";       
   ...
   <script DEFER>
            ...
            for(int i=0; i<objArrayContactList.size(); i++ ) {
              ...
              fxCreateRowContact();
               ...
               if (index == 1){  
                ...
                <%// if (MiUtil.getString(objContactBean.getNpaction()).trim().equals("Nuevo") || MiUtil.getString(objContactBean.getNpaction()).trim().equals("Eliminado"))                {%>
                    //       form.cmbEstatus.disabled = true;
                <%//}%>    
                ...
                <%}%>   
                 ...
                 //form.txtEstado[index-1].value      = "<%=MiUtil.getString(objContactBean.getNpaction())%>";                  
                 //alert(form.cmbCargo[<%=i%>].options[form.cmbCargo[<%=i%>].selectedIndex].text);
                ...
                /* VALORES ANTIGUOS EN EL FORMULARIO DE LA SECCION */
                 ...
                 <%// if (MiUtil.getString(objContactBean.getNpaction()).trim().equals("Nuevo") || MiUtil.getString(objContactBean.getNpaction()).trim().equals("Eliminado")) {%>
                           //form.cmbEstatus[index-1].disabled = true;
                 <%//}%>
     ...
     function fxOnChangeStatus(objValue ,objIndex){
     
        var numRows = table_assignment.rows.length;
        var form    = document.frmdatos;
        var index   = objIndex - 1;
      
        if (numRows == 2 && form.cmbEstatus.value != undefined ){              
              form.hdnNpaction.value = form.cmbEstatus.value;
             // alert(index+' - = 2- '+form.hdnNpaction.value);       
          
        }else{
          form.hdnNpaction[index].value = form.cmbEstatus[index].value;
          //alert(index+' - > 2 - '+form.hdnNpaction[index].value);         
        }     
     }
