  ...
  <head>
    ...
    <%  HashMap hshDataMap = (HashMap) request.getAttribute(Constante.DATA_STRUCT);
		...
		if(StringUtils.isBlank(strMessage)) {
			ArrayList arrCategoryList = (ArrayList) hshDataMap.get("arrCategoryList");
        	Iterator iterator = arrCategoryList.iterator();
        	while(iterator.hasNext()) {
            	DominioBean dominio = (DominioBean) iterator.next();
    %>			fxAddNewOption(parent.mainFrame.document.formdatos.cmbCategoria,'<%=dominio.getValor()%>','<%=dominio.getDescripcion()%>');
    <%  	}
		} else {
	%>		alert('<%="Hubo error en la carga de la Categoría: "+MiUtil.getMessageClean(strMessage)%>');
	<%	}
    %>
