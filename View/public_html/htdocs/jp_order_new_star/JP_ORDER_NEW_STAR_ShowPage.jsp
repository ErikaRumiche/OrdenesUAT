...
//INICIO PRY-0864 | AMENDEZ --Se adiciono la especificacion 2009
 function fxInvocarCreditEvaluation(npspecificationid, npgeneratortype){
     ...
     try{
      ...
      if (((npspecificationid == '2001' && campVar != "C") ||npspecificationid == '2068' || npspecificationid == '2013' || npspecificationid == '2017' || npspecificationid == '2065' || npspecificationid == '<%=Constante.SPEC_SSAA_SUSCRIPCIONES%>' || npspecificationid == '<%=Constante.SPEC_SSAA_PROMOTIONS%>' || npspecificationid == '2024') && (npgeneratortype!= 'MASSIVE')) {
        form.myaction.value="evaluarOrden"; 
        form.action="<%=actionURL_OrderServlet%>";          
        form.submit();
        return false;
...
<!-- Por implementar -->
  ...
  function fxValidateSelectionRPag (opcion){
    ...
    siteSecRespPago  = form.hdnSiteSecRespPago.value;
        ...
        if((form.cmbSubCategoria.options[form.cmbSubCategoria.selectedIndex].value == 2082 ||
                form.cmbSubCategoria.options[form.cmbSubCategoria.selectedIndex].value == 2083 ||
                form.cmbSubCategoria.options[form.cmbSubCategoria.selectedIndex].value == 2084 ||
                form.cmbSubCategoria.options[form.cmbSubCategoria.selectedIndex].value == 2085) &&
                numRespPago == 0){
            alert("Debe seleccionar un Responsable de Pago");
            form.cmbResponsablePago.focus();
            return true;
  ...
  function fxEditAdress() {
      ...
      if (customerid != "") {
        var param = "?strDirEntrega="+form.hdnDeliveryAddress.value+"&strDpto="+form.hdnDeliveryState.value+"&strProv="+form.hdnDeliveryProvince.value+
                    ...
                    "&strReference="+form.txtReference.value; // [N_O000017567] MMONTOYA
        var param = param+"&strSpecification="+form.cmbSubCategoria.value+"&strUserName="+form.hdnSessionLogin.value; //[SP3103 VALIDACION DIRECCIONES RIESGO] DIANA LLANOS
        var param = param+"&strOrderId="+form.hdnNumeroOrder.value+"&strCompanyId="+form.txtCompanyId.value; //[SP3103 VALIDACION DIRECCIONES RIESGO] DIANA LLANOS
    ...
    function validateCourierDelivery(){
    	...
    	specificationDely = form.cmbSubCategoria.value;
    	        	...
    	        	if(chkCourier == '1') {
    	        		...
    	        		$.ajax({ 
    	                    ...
    	                    data:{
    	                        ...
    	                        "cmbLugarDespacho":cmbLugarAtencion,
