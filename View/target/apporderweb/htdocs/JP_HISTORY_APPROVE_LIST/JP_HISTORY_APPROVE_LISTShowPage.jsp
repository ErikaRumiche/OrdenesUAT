...
try {
	...
	ArrayList arrHistoryApproveList = objOrderTabsService.getHistoryApproveListByOrder(lOrderId);
    System.out.println("Tamaño del ArrayList: " + arrHistoryApproveList.size());
	if(arrHistoryApproveList!=null) {
		request.setAttribute("arrHistoryApproveList", arrHistoryApproveList);
	}
%>
...
<form name="frmdatos" action="<%=request.getContextPath()%>/orderSearchServlet" method="POST">
	...
	<display:table name="arrHistoryApproveList" class="orders" export="true" pagesize="100" style="width: 100%">
		<display:column property="swagent" title="Enviado por"/>
		...
		<display:column property="swactionrequired" title="swactionrequired"/>
		<display:column property="swconfirmrequested" title="swconfirmrequested"/>
		--%>
		<display:column property="npvaluedesc" title="Estado"/>
