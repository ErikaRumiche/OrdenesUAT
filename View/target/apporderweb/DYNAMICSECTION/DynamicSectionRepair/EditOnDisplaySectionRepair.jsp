...
try {   
  ...
  String strdetResolCentro="";
   ...
   // puedo validar el fono venido de la validación previa para usar el valor obtenido amteriormemte
  ...
  System.out.println("strObsAsesorInterna: "+strObsAsesorInterna);
   ...
   /*
    * El botón Generar Modelo solo debe estar habilitado cuando el modelo no existe en BSCS
    */
   ...
   if (strNpServicioNac != null) { 
       ...
       // Si el modelo existe, el botón se deshabilita.
...
<script type="text/javascript">
   ...
   function fxvalidateCharacteres(){
      ...
      if (cantidad > max){
         alert("La cantidad máxima de caracteres es 4000");
  ...
  function fxCargaHiddenFlagValidateTypeImei(){
   ...
   //funcion para validar Celular de contacto, sólo numero, 9 digitos y que el primer digito no sea 0
   ...
   <tr>
    ...
    <tr>
        ...
        <tr>
              ...
              <td class="CellLabel" align="center" height="41">Código OSIPTEL</td>
...
<script>
  ...
  var arreglo = <%=Constante.SPEC_LOAD_CMB_PROCESS%>;
    ...
    if ((strRepairType == "REP") && (cmbProceso == "REX")){
        ...
        if (numValidChangeProcess > 0){
          ...
          alert("Existen repuestos asociados a la reparación, debe eliminar estos antes de cambiar a REX.");
          return false;
        }
    }          
    ...
    // validación para prender la sección del boton de nacionalización
    if (("SERVICIO" == cmbProceso) && (wNpServiceCode == "Nacionalización de Equipo")){
  ...
  function fxChangeProcessRepairList(){
    ...
    function fxAlertDocGeneration(){
        return confirm("Asegurar el ingreso en la orden de toda la información requerida por el documento. Una vez generado el documento no se podrá volver a generar.\n ¿Desea generar los documentos digitales?");
    }
...
function fxGenerateNacionalizacion() {   
  ...
  function fxGenerateRAFile() {   
    ...
    if (confirm("¿Está seguro que desea enviar el correo para registrar el Modelo?")) {
