...
try{
 //cirazabal
  boolean  isTelefonia; 
  String strCheckDefault     = null;
  String strCheckDefaultSet  = null;
   ...
   if(strSpecificationId.equals(String.valueOf(Constante.KN_VTA_TELEFONIA_FIJA_BA))){
    isTelefonia = true;      
   }else{
    isTelefonia = false;      
   }
   ...
   if (strResultGross != null && strResultGross.equals("1")){
      if( MiUtil.getString(strGeneratorType).equals(Constante.GENERATOR_TYPE_OPP) || MiUtil.getString(strGeneratorType).equals(Constante.GENERATOR_TYPE_OPP_PORTAB)  ){//Si el origen es una OPP o OPP_PORTAB
...
<tr>
   ...
   <td align="right" id="sitenew" >
      ...
      /*Cambio cirazabal   15/07/2009 - Validacion para Telefonia Fija VoIP*/
      /*JPEREZ 05/08/2009 Cambio VoIP: Permite crear sites para clientes Prospect que no hayan creado sites en la oportunidad */
      //if (iLongitudNew == 0){
      if (iLongitudNew <= 0 && !strSpecificationId.equals(String.valueOf(Constante.KN_VTA_TELEFONIA_FIJA_BA))){     
      %>
      ...
      <%}else if (iLongitudNew <= 0 && strSpecificationId.equals(String.valueOf(Constante.KN_VTA_TELEFONIA_FIJA_BA)) && objCustomerBean.getSwType().equalsIgnoreCase("Prospect") && lngUnknwnSiteId == 0 ) {
      %>
        <a href="javascript:fxAddNewSite();"> 
        <img name="item_img0" Alt="Agregar Site" src="<%=Constante.PATH_APPORDER_SERVER%>/images/crear.gif" border="no">
        </a>               
   ...
   <td>             
      <table align=center width="100%" border="0" id="tabSite" name="tabSite" cellpadding="0" cellspacing="1" >
         ...
         <td class="CellLabel" width="5%">#</td>
          <%if(isTelefonia && lSiteIncId == 0){%>   
             <td class="CellLabel" width="5%">Cargo a</td>
          <%}%>   
      ...
      <%if(isTelefonia && lSiteIncId == 0){
         ...
         if(iLongitudNew == 1){
          ...
          strCheckDefaultSet = MiUtil.getString((String)hshSites.get("swsiteid"));
         }else if (iLongitudNew > 1 && String.valueOf(hshSites.get("swsolutiongroup")).equals("3")){
          strCheckDefault = "checked";
          strCheckDefaultSet = MiUtil.getString((String)hshSites.get("swsiteid"));
         }
      ...
      %>      
        <td class="CellContent" >
           <input type='radio' id='idSiteCheck' name='siteCheck' onclick="fxCheckSite(this);"  value = "<%=MiUtil.getString((String)hshSites.get("swsiteid"))%>" >
        </td>
      <%}
      else{
        strCheckDefault = "";
...
<script language='javascript' DEFER>   
   ...
   var flagOPP = 0;
   //cirazabal: variables para controlar la selección de un Site de la sección de Responsables de Pago
   var flagSiteCheck = false;
   try{
    var countUnknownSites = <%=arrNewSite.size()%>;
   }catch(e){} 
   ...
   function fxSiteOnLoad(){            
      try{
        ...
        var defaultset= "";
        try{
          defaultset = "<%=strCheckDefaultSet%>";          
          form.hdnSiteSecRespPago.value = "";          
       }catch(e){}
       ...
       if( defaultset != ""){
         ...
         try{            
            form.hdnSiteSecRespPago.value = defaultset;                    
        }catch(e){}        
        }        
      }
      catch(e){}
   ...
   function fxSectionNameValidate22(){
      ...
      return true      
       ...
       function fxCheckSite(control){
      try{
        form = document.frmdatos;
       if (form.cmbResponsablePago.value != ""){ 
           form.cmbResponsablePago.selectedIndex = ";";
           form.txtCodBSCS.value = form.hdnCodBSCS.value;
           } 
        //Actualizamos el flag
        flagSiteCheck = true;
        //Actulizamos el Site elegido
        form.hdnSiteSecRespPago.value = control.value;
      }catch(e){}
    }
   ...
   function fxAddSite(id,siteName,regionName,status,createdBy){
      ...
      col = row.insertCell(-1);
      col.className = "CellContent";      
      col.innerHTML = "<input type='radio' id='idSiteCheck' name='siteCheck'  onclick='fxCheckSite(this);' value = '"+id+"'>";
   ...
   function fxValidSelectSite(){
    ...
    function fxSelectRadioDefault(){    
      try{
          var respPagoSelectedCompania = form.cmbResponsablePago.value;
          var numRespPagoCompania      = form.cmbResponsablePago.length - 1; 
          if (countUnknownSites >= 1 && respPagoSelectedCompania=='') {
          //Solo existe un site unknown y no hay seleccion en el combo de Sites, se selecciona por defecto.
            if(countUnknownSites == 1){          
              try{
                
                document.frmdatos.siteCheck.checked = true;
                fxCheckSite(document.frmdatos.siteCheck);
                
              }catch(e){}
            }else{
              try{
                
                document.frmdatos.siteCheck[0].checked = true;
                fxCheckSite(document.frmdatos.siteCheck[0]);
    ...
    //fxSelectRadioDefault();
    
    function fxSectionNameValidateSelectSite(){
      form = document.frmdatos;
      var respPagoSelectedCompania = form.cmbResponsablePago.value;
      var numRespPagoCompania      = form.cmbResponsablePago.length - 1;      
      
      //Verifica la existencia de elementos en el combo de sites
      if('<%=Constante.KN_VTA_TELEFONIA_FIJA_BA%>' == '<%=strSpecificationId%>'){
        if( (numRespPagoCompania <= 1 && respPagoSelectedCompania=='')||(numRespPagoCompania > 1 && respPagoSelectedCompania=='') ){
          if(form.cmbFormaPago.value == '<%=Constante.PAYFORM_CARGO_EN_RECIBO%>'){
            if( !fxValidSelectSite() ){
              alert("Debe seleccionar un Site para el cargo del recibo ");
              return false;
            }
          }
        }
